<script>
function crear() {
$('#result').load('inso_incidencia_seg_op_crear.php'
,
{
    'i_inso_incidencia':  $('#i_inso_incidencia').val()
    }
    ,
    function(){
        $('#modal').hide('slow');
...
function modificar() {
$('#result').load('inso_incidencia_seg_op_modificar.php?id=' + $('#h2_id').val()
,
{
     'm_inso_id':  $('#m_inso_id').val(),
     'm_inso_incidencia':  $('#m_inso_incidencia').val()
    }
    ,
    function(){
       $('#modal2').hide('slow');
...
if(agree) {
   $('#result').load('inso_incidencia_seg_op_borrar.php?id=' + id
   ,
   function()
     {
     mostrar();
...
$('#modal2').center();
$('#h2_id').val(id);
$.get('inso_incidencia_seg_op_datos.php?id=' + id, function(data){
     var resp=data;
     r_array = resp.split('||');
     //alert(r_array[0]);
     $('#m_inso_incidencia').val(r_array[1]);
     });
}
function mostrar() {
$('#datos_mostrar').load('inso_incidencia_seg_op_mostrar.php');
}
</script>
<div id='separador'>
...
<td><input type='text' id=i_inso_incidencia size=40 class='entradas'></td>
</tr>
<tr>
...
<tr>
<td class='etiquetas'>Tipo de incidencia:</td>
<td><input type='text' id=m_inso_incidencia size=40 class='entradas'></td>
</tr>
<tr>
