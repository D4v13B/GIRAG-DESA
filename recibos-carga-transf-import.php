...
<section class="content">
   <div class="card border">
      ...
      <form class="card-body" id="form-nueva-carga">
         <button type="submit" class="btn btn-success btn_guardar_trans_import"><i class="fa-solid fa-floppy-disk"> </i><?php echo isset($_GET["carg_id"]) ? " GUARDAR" : " GUARDAR" ?></button>
         ...
         <script>
            ...
            //       $(".btn_guardar_trans_import").show();
            //    <?php else: ?>
            //       // Si el estado es distinto de 1, ocultar el botón y aplicar roles
            //       $(".btn_guardar_trans_import").hide()
            //       <?php echo pantalla_roles("index.php?p=recibos-carga-transf-import", $_SESSION["login_user"]) ?>
            //    <?php endif; ?>
            // });
         </script>
               ...
               <fieldset class="form-group row">
                  ...
                  <div class="col-sm-10">
                     <?php while ($fila = mysql_fetch_assoc($carga_tipos)) : ?>
                        <?php if ($fila['cati_id'] != 3) continue; // Mostrar solo el ID 3 TRANSF IMPORTACION 
                        ?>
      ...
      <div class="modal-content">
         ...
         <div class="modal-body">
            <form id="addForm">
               ...
               <div class="form-group c_consignee" style="display:none;">
                  <label for="ciudadinput">Email</label>
                  <input type="text" class="form-control" id="ciudadinput" name="i_cons_email">
               </div>
               <div class="form-group c_consignee" style="display:none;">
                  <label for="ciudadinput">Teléfono</label>
                  <input type="text" class="form-control" id="ciudadinput" name="i_cons_telefono">
               </div>
...
<div class="modal fade" id="modalCajaDetalles" tabindex="-1" aria-hidden="true">
   <div class="modal-dialog " style="max-width: 90%; width: auto;;">
      <div class="modal-content">
         ...
         <div class="modal-body">
            <div class="row">
               <h6 class="text-bold">Datos del transportista</h6>
               <div class="d-flex w-100 justify-content-between">
                  <div class="form-group col">
                     <input type="text" class="disabled-input-fact transporte-update form-control" data-campo="cade_transportista_nombre" data-cadeId="" name="inpNombre" placeholder="Nombre">
                  </div>
                  <div class="form-group col">
                     <input type="text" class="disabled-input-fact transporte-update form-control" data-campo="cade_transportista_cedula" data-cadeId="" name="inpCedula" placeholder="Cédula/RUC">
                  </div>
                  <div class="form-group col">
                     <input type="text" class="disabled-input-fact transporte-update form-control" data-campo="cade_transportista_matricula" data-cadeId="" name="inpMatricula" placeholder="Matrícula">
                  </div>
               </div>
            </div>
            <h6 class="text-bold">Cargos</h6>
...
function registrarCargadetalle() {
    ...
    $.ajax({
        ...
        success: function(res) {
            ...
            // Buscar el primer '{' para ignorar logs antes del JSON
            ...
            // Si hubo error en PHP (guía duplicada, etc)
            if (!data.success) {
                alert(data.message || "Ocurrió un error desconocido.");
                return;
            }

            // Si todo sale bien, reseteamos formulario y obtenemos detalles
               ...
               if (!detalles) {
                  ...
                  detalles.forEach(detalle => {
                     const row = `
                        ...
                        `;
                        // <button type="button" class="btn btn-danger btn-borrar btn-sm" onclick="eliminarCargaDetalle(${detalle.cade_id})" hidden>
                        //                <i class="fas fa-trash"></i>
                        //             </button>
                        //             <button type="button" class="btn btn-info btn-sm" data-toggle="modal" data-target="#modalCajaDetalles" onclick="mostrarCajaDetalles(${detalle.cade_id}, null, ${detalle.cade_guia})" data-title='SERVICIOS' title='SERVICIOS'>
                        //                <i class="fa-solid fa-cash-register"></i>
                        //             </button>
                        //             <button type="button" class="btn btn-success btn-sm" data-toggle="modal" data-target="#modalCajaDetalles" onclick="mostrarCajaDetalles(${detalle.cade_id}, 1, ${detalle.cade_guia})" data-title='FACTURAR' title='FACTURAR' hidden>
   ...
   function mostrarModalTransportista(cade_id) {
      ...
      $.ajax({
         ...
         success: function(response) {
            ...
            // Populate modal fields if transportista details exist
      ...
      $.ajax({
         ...
         success: res => {
            ...
            if (res.length == 0) {
               ...
               $("#cons_id").val(res.detail_info.cons_id)
   ...
   function guardarServicios() {
      ...
      data.append('cons_id', $("#cons_id").val())
      $.ajax({
   ...
   function facturar() {
      $.post(`./ajax/cargos_servicios.php?ruc=${$("input[name='inpConsRuc']").val()}&tipo=2&dv=${$("input[name='inpConsDv']").val()}`, {
