...
<div class='table-responsive table-striped table-bordered table-hover table-sm' style='text-align: center; align-items:center'>
   <table id='resultado' class=table align-middle>
      ...
      <tbody>
         <?php
         $f_cati_id = $_GET['f_cati_id'];
         $f_carg_guia = $_GET['f_carg_guia'];
         $f_vuel_codigo = $_GET['f_vuel_codigo'];
         $f_aeco_id_destino_final = $_GET['f_aeco_id_destino_final'];
         $f_usua_id_creador = $_GET['f_usua_id_creador'];
         $f_carg_fecha_registro = $_GET['f_carg_fecha_registro'];
         $f_carg_recepcion_real = $_GET['f_carg_recepcion_real'];
         $f_liae_id = $_GET['f_liae_id'];
         $f_caes_id = $_GET['f_caes_id'];
         
         $where = '';
         if ($f_cati_id != '' && $f_cati_id != 'null') $where .= "AND a.cati_id IN ($f_cati_id)";
         //if ($f_carg_guia != '' && $f_carg_guia != 'null') $where .= "AND a.carg_guia LIKE '%$f_carg_guia%'";
		 if ($f_carg_guia != '' && $f_carg_guia != 'null') $where .= "AND a.carg_id in (select carg_id from carga_detalles where cade_guia LIKE '%$f_carg_guia%')";
         if ($f_vuel_codigo != '' && $f_vuel_codigo != 'null') $where .= "AND g.vuel_codigo LIKE '%$f_vuel_codigo%'";
         if ($f_aeco_id_destino_final != '' && $f_aeco_id_destino_final != 'null') $where .= "AND a.aeco_id_destino_final IN ($f_aeco_id_destino_final)";
         if ($f_usua_id_creador != '' && $f_usua_id_creador != 'null') $where .= "AND a.usua_id_creador IN ($f_usua_id_creador)";
         
         // Corrección para el filtro de fecha de registro
         if ($f_carg_fecha_registro != '' && $f_carg_fecha_registro != 'null') {
            // Convertir el formato YYYYMMDD a YYYY-MM-DD
            $fecha_formateada = substr($f_carg_fecha_registro, 0, 4) . '-' . 
                              substr($f_carg_fecha_registro, 4, 2) . '-' . 
                              substr($f_carg_fecha_registro, 6, 2);
            $where .= " AND DATE(a.carg_fecha_registro) = '$fecha_formateada'";
         }
         
         // Filtro para fecha de recepción
         if ($f_carg_recepcion_real != '' && $f_carg_recepcion_real != 'null') {
            // Convertir el formato YYYYMMDD a YYYY-MM-DD
            $fecha_recepcion_formateada = substr($f_carg_recepcion_real, 0, 4) . '-' . 
                                        substr($f_carg_recepcion_real, 4, 2) . '-' . 
                                        substr($f_carg_recepcion_real, 6, 2);
            $where .= " AND DATE(a.carg_recepcion_real) = '$fecha_recepcion_formateada'";
         }
         
         if ($f_liae_id != '' && $f_liae_id != 'null') $where .= "AND a.liae_id IN ($f_liae_id)";
         if ($f_caes_id != '' && $f_caes_id != 'null') $where .= "AND a.caes_id IN ($f_caes_id)";

         $qsql = "SELECT a.*, b.cati_nombre, c.aeco_nombre, d.usua_nombre, 
                        e.liae_nombre, f.caes_nombre, g.vuel_codigo, f.caes_id
                 FROM carga a
                 JOIN carga_tipos b ON a.cati_id=b.cati_id
                 JOIN aereopuertos_codigos c ON a.aeco_id_destino_final=c.aeco_id
                 JOIN usuarios d ON a.usua_id_creador=d.usua_id
                 JOIN lineas_aereas e ON a.liae_id=e.liae_id
                 JOIN carga_estado f ON a.caes_id=f.caes_id
                 JOIN vuelos g ON a.vuel_id=g.vuel_id
                 WHERE 1=1 $where
                 ORDER BY a.carg_fecha_registro DESC";
		//echo nl2br($qsql);
         $rs = mysql_query($qsql);
         ...
         while ($i < $num) {
         ?>
            <tr class='tabla_datos_tr' <?php 
               switch(mysql_result($rs, $i, 'caes_id')){
                  case "3":
