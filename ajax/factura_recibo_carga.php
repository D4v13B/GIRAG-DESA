...
/**
 * Me trae todos los detalles agrupados por guia
 */
$sql = "SELECT cd.*, cl.calb_nombre
FROM carga_detalles cd
LEFT JOIN carga_detalle_tipo cdt ON cd.cade_tipo_id = cdt.cade_tipo_id
LEFT JOIN carga_localizaciones_bodega cl ON cd.calb_id = cl.calb_id
WHERE cd.carg_id = $carg_id";
if ($cade_id !== null) {
    $sql .= " AND (cd.cade_id = $cade_id OR cd.cade_guia = $cade_id)";
}
...
$desc .= "<tr style='background-color:#101a21; font-family: Tahoma, Geneva, sans-serif;'>
    ...
    <td style='width: 70px;font-size:14px; color:#ffffff'>$t_kg</td>
