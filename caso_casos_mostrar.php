...
<div class="table-responsive table-striped table-bordered table-hover table-sm" style="text-align: center; align-items:center">
   <table class="table align-middle">
      ...
      <tbody>
		 ...
		 $cape_id = $_GET["cape_id"];
         $liae_id = $_GET["liae_id"];
		 ...
		 if ($cape_id != "") $where .= " AND a.cape_id IN ($cape_id)";
		 if ($liae_id != "") $where .= " AND a.liae_id IN ($liae_id)";
         ...
         if ($_SESSION["administrador_caso"] == 0) 
		 {
			...
			LEFT JOIN 
                lineas_aereas la ON a.liae_id = la.liae_id
			LEFT JOIN 
                casos_peligros cp ON a.cape_id = cp.cape_id
            WHERE 
		 ...
		 {
            $qsql = "SELECT caso_fecha_analisis, caes_id, caso_id, caso_descripcion, cati_nombre, inso_nombre, inpr_nombre, 
			caso_fecha_creacion, depa_nombre, caso_ubicacion, imec_nombre, imma_nombre, equi_nombre, caso_fecha, caso_nota, 
			impe_nombre, usua_id_aprobado, usua_id_revisado,caso_fecha_cierre,usua_id_encargado_revision, usua_id_encargado_aprobacion,
         (SELECT liae_nombre FROM lineas_aereas WHERE liae_id = a.liae_id) liae_nombre,
		 (SELECT cape_nombre FROM casos_peligros WHERE cape_id = a.cape_id) cape_nombre,
         ...
         ?>
            <tr>
			   ...
			   <td><?php echo mysql_result($rs, $i, 'liae_nombre'); ?></td> 
			   <td><?php echo mysql_result($rs, $i, 'cape_nombre'); ?></td> 
