...
<section class="content">
   <div class="card border">
      ...
      <form class="card-body" id="form-nueva-carga">
         <button type="submit" class="btn btn-success btn_guardar_export"><i class="fa-solid fa-floppy-disk"> </i><?php echo isset($_GET["carg_id"]) ? " GUARDAR" : " GUARDAR" ?></button>
         ...
         <!-- <script>
            $(function() {
               ...
               <?php if ($caes_id == 1): ?>
                  ...
                  $(".btn_guardar_export").show();

               <?php else: ?>

                  // Si el estado es distinto de 1, ocultar el botón y aplicar roles

                  $(".btn_guardar_export").hide();

                  <?php echo pantalla_roles("index.php?p=recibos-carga-export", $_SESSION["login_user"]); ?>
         ...
         <div class="form-row">
            ...
            <!-- <div class="form-group col-3">
               ...
               <select id="inputAgencia" class="form-control custom-select" name="agencia">
                  ...
                  <?php while ($fila = mysql_fetch_assoc($lineas_aereas)) : ?>
                     <option <?php if (isset($res['liae_id']) and $fila["liae_id"] == $res["liae_id"]) {
                                 echo 'selected';
                              } ?> value="<?php echo $fila["liae_id"] ?>"><?php echo $fila["pais_nombre"] . "/" . $fila["liae_nombre"] ?></option>
                  <?php endwhile ?>
               </select>
            </div> -->
            ...
            <div class="form-group col-3">
               ...
               <select id="inputOrigen" class="form-control custom-select" name="carg_origen">
                  <option value="" disabled <?php echo (!isset($res['carg_origen']) || empty($res['carg_origen'])) ? 'selected' : ''; ?>>Seleccione una opción</option>
                  <?php
                  mysql_data_seek($paises, 0);
                  while ($fila = mysql_fetch_assoc($paises)) : ?>
                     <option value="<?php echo $fila["aeco_id"]; ?>" <?php echo (isset($res['carg_origen']) && $fila["aeco_id"] == $res["carg_origen"]) ? 'selected' : ''; ?>>
            ...
            <div class="form-group col-3">
               ...
               <select id="inputVuelo" class="form-control custom-select" name="vuelo">
                  <option value="<?php echo $res['vuel_id'] ?? ''; ?>" disabled <?php echo (!isset($res['vuel_id']) || empty($res['vuel_id'])) ? 'selected' : ''; ?>>Seleccione una opción</option>
                  <?php
            ...
            <div class="form-group col-3">
               ...
               <div class="select-container">
                  ...
                  selectShipper($ship_id, $ship_nombre) ?>
               ...
               <fieldset class="form-group row">
                  ...
                  <div class="col-sm-10">
                     ...
                     <?php while ($fila = mysql_fetch_assoc($carga_tipos)) : ?>
                        ...
                        <?php if ($fila['cati_id'] != 2) continue; // Mostrar solo el ID 2 
...
$(document).ready(function() {
   ...
   // 1. OBTENER EL ROL DEL USUARIO DESDE PHP
    const userRoleId = <?php echo $usti_id_actual; ?>;
    const isSupervisorOrSuperUser = (userRoleId === 0 || userRoleId === 5);
    
    // 2. VERIFICAR ESTADO AL CARGAR LA PÁGINA
    verificarEstadoCarga();
    
    // 3. FUNCIÓN PARA VERIFICAR SI LA CARGA YA ESTÁ RECIBIDA
        ...
        $.ajax({
            ...
            success: function(respuesta) {
                ...
                if (respuesta.success && respuesta.estado == 2) {
                    ...
                    // La carga está recibida (estado == 2) Y el usuario NO es Supervisor/SuperUsuario
                    if (!isSupervisorOrSuperUser) {
                        deshabilitarFormulario();
                    } else {
                        // Opcional: Mostrar un mensaje a los Super Usuarios/Supervisores de que la carga está Recibida
                        console.log("Carga recibida, pero usuario autorizado para editar.");
                    }
                }
            }
        });
    ...
    // 4. FUNCIÓN PARA DESHABILITAR EL FORMULARIO (Se mantiene igual)
    function deshabilitarFormulario() {
        // Deshabilitar todos los elementos
        $('input, select, textarea, button').prop('disabled', true);
        
        // Excepción: Permitir la impresión
        $('a[href*="factura_recibo_carga_export.php"]').prop('disabled', false).removeClass('disabled');
