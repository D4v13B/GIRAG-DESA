...
<th class=tabla_datos_titulo>Pertenece a</th>
<th class=tabla_datos_titulo_icono></th>
</tr>
</thead>
<tbody>
<?php
$f_casu_descripcion=$_GET['f_casu_descripcion'];
$f_cacl_id=$_GET['f_cacl_id'];
$where='';
if($f_casu_descripcion!='' && $f_casu_descripcion!='null') $where .="AND a.casu_descripcion LIKE '%$f_casu_descripcion%'";
if($f_cacl_id!='' && $f_cacl_id!='null') $where .="AND a.cacl_id IN ($f_cacl_id)";

$qsql ="select * from casos_subclasificacion a,casos_clasificacion b
WHERE 1=1 
AND a.caso_id=b.caso_id
$where
";
...
<td class=tabla_datos><?php echo mysql_result($rs, $i, 'casu_descripcion'); ?></td>
<td class=tabla_datos><?php echo mysql_result($rs, $i, 'cacl_nombre'); ?></td>

            <td class=tabla_datos_iconos>
            <div Class='btn-group btn-group-sm'>
                     <a Class='btn' href='javascript:editar(<?php echo mysql_result($rs, $i, 'casu_id'); ?>)' ;>
                     ...
                     <a Class='btn' href='javascript:borrar(<?php echo mysql_result($rs, $i, 'casu_id'); ?>)' ;>
