...
$sql = "SELECT * FROM contratos WHERE cont_nombre = 'RECIBO-CARGA'";
...
if ($cade_id !== null) {
    ...
    if ($details_result) {
        ...
        // $plantilla = str_replace("[TRANSPORTISTA]", $res["cade_transportista_nombre"], $plantilla);
...
$plantilla = str_replace("[KILOS_TOTALES]", $res["peso_total"], $plantilla);
...
// INNER JOIN carga_detalle_localizacion cdl ON cdl.cade_id = cd.cade_id
// INNER JOIN carga_localizaciones_bodega clb ON cdl.calb_id = clb.calb_id


$sql = "SELECT *,
       (SELECT GROUP_CONCAT(DISTINCT cl.calb_nombre SEPARATOR ', ')
        FROM carga_localizaciones_bodega cl,
             carga_detalle_localizacion cdl
        WHERE cdl.calb_id = cl.calb_id
          AND cdl.cade_id = cd.cade_id) calb_nombre
FROM carga_detalles cd
         INNER JOIN carga_detalle_tipo cdt ON cd.cade_tipo_id = cdt.cade_tipo_id
...
if ($cati_id == 1 || $cati_id==3) {  //SI ES IMPORT, TIENE LA COLUMNAS DE CARACTERISTICAS FISICAS
	...
	<thead>
        ...
        <th style="background-color:#101a21; text-align:center"><span style="font-size:14px"><span style="font-family:Tahoma,Geneva,sans-serif"><span style="color:#ffffff">Peso KG</span></span></span></th>			
            <th style="background-color:#101a21; text-align:center"><span style="font-size:14px"><span style="font-family:Tahoma,Geneva,sans-serif"><span style="color:#ffffff">Localización</span></span></span></th>
            <th style="background-color:#101a21; text-align:center"><span style="font-size:14px"><span style="font-family:Tahoma,Geneva,sans-serif"><span style="color:#ffffff">Descripción</span></span></span></th>
            <th style="background-color:#101a21; text-align:center"><span style="font-size:14px"><span style="font-family:Tahoma,Geneva,sans-serif"><span style="color:#ffffff">Tipo de carga</span></span></span></th>
		</tr>
	</thead>
	<tbody>';
} else { //SI ES EXPORT, TIENE UNA SOLA  FORMA SIN LAS DIMENSIONES
    $desc .= '<table cellspacing="0" style="border-collapse:collapse; margin:10px auto; text-align:left; width:99%">
	<thead>
		<tr>
		<th style="background-color:#101a21; text-align:center"><span style="font-size:14px"><span style="font-family:Tahoma,Geneva,sans-serif"><span style="color:#ffffff">Piezas</span></span></span></th>
...
foreach ($res as $index => $item) {
    ...
    $noImportInfo = '';
    if ($cati_id == 1 || $cati_id==3) { // SI ES IMPORT VAMOS A PONER LA PIEZA, EL PESO, LOCALIZACION, DESCRIPCION, TIPO DE CARGA
        $rowStyle = ($index % 1 == 0) ? 'background-color: #D3D3D3;' : 'background-color: #D3D3D3;';
        ...
        <td class='tabla-detalles' style='font-size:14px'>" . $item["cade_peso"] . "</td>
        <td class='tabla-detalles' style='font-size:14px'>" . $item["calb_nombre"] . "</td> 
        <td class='tabla-detalles' style='font-size:14px'>" . $item["cade_desc"] . "</td>
        <td class='tabla-detalles' style='font-size:14px'>" . $item["cade_descripcion"] . "</td>
        ...
        $t_volumen += $volumen;
        $t_piezas += $item["cade_piezas"];
        $t_kg += $item["cade_peso"];
    }
}
$desc .= "<tr style='background-color:#101a21; font-family: Tahoma, Geneva, sans-serif;'>
    <td style='width: 50px;font-size:14px; color:#ffffff'>$t_piezas</td>
    <td style='width: 65px;font-size:14px'></td>
    <td style='width: 65px;font-size:14px'></td>
	<td style='width: 65px;font-size:14px'></td>
	<td style='width: 65px;font-size:14px'></td>
    </tr>";

// <td style='width: 50px;font-size:14px; color:#ffffff'>" . number_format($t_volumen, 2) . "</td>
